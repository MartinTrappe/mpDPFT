...
#define localPI 3.1415926535897932384626433832795029
...
void cec14_test_func(double *x, double *f, int nx, int mx, int func_num, /*MIT*/int TestMode)
{
	int cf_num=10,i,j;
	if (ini_flag==1)
	{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "CEC2014_input_data/M_%d_D%d.txt", func_num,nx);
		...
		if (func_num<23)
		{
			...
			for (i=0; i<nx*nx; i++)
			{
				//fscanf(fpt,"%lf",&M[i]);
				//MIT:
				if(fscanf(fpt,"%lf",&M[i])!=1) printf("cec14_test_func --- loading error for M");
			}
			//MIT, to print rotation matrix elements:
			//printf("{"); for(i=0;i<nx*nx-1;i++) printf("%1.16f,",M[i]); printf("%1.16f",M[nx*nx-1]); printf("}\n\n");
		}
		else
		{
		...
		/* Load shift_data */
		sprintf(FileName, "CEC2014_input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		if (func_num<23)
		{
			...
			for(i=0;i<nx;i++)
			{
				...
				//MIT:
				if(fscanf(fpt,"%lf",&OShift[i])!=1) printf("cec14_test_func --- loading error for OShift");
			}
			//MIT, to print shift elements:
			//printf("{"); for(int j=0;j<nx-1;j++) printf("%1.16f,",OShift[j]); printf("%1.16f",OShift[nx-1]); printf("}\n\n");
		}
		else
		{
			...
			for(i=0;i<cf_num-1;i++)
			{
				for (j=0;j<nx;j++)
				{
					...
					//MIT:
					if(fscanf(fpt,"%lf",&OShift[i*nx+j])!=1) printf("cec14_test_func --- loading error for OShift");
				}
				//fscanf(fpt,"%*[^\n]%*c"); 
				//MIT:
				int dummy = fscanf(fpt,"%*[^\n]%*c"); //printf("cec14_test_func --- dummy = %d",dummy);
			}
			for (j=0;j<nx;j++)
			{
				//fscanf(fpt,"%lf",&OShift[(cf_num-1)*nx+j]);
				//MIT:
				if(fscanf(fpt,"%lf",&OShift[(cf_num-1)*nx+j])!=1) printf("cec14_test_func --- loading error for OShift");
			}
		...
		{
			sprintf(FileName, "CEC2014_input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
			...
			for(i=0;i<nx;i++)
			{
				//fscanf(fpt,"%d",&SS[i]);
				//MIT:
				if(fscanf(fpt,"%d",&SS[i])!=1) printf("cec14_test_func --- loading error for SS");				
			}	
			fclose(fpt);
		}
	...
	for (i = 0; i < mx; i++)
	{
		...
		case 22:	
			...
			break;
		case 23:	
			cf01(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2300.0;
			break;
		case 24:	
			cf02(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2400.0;
			break;
		case 25:	
			cf03(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2500.0;
			break;
		case 26:
			cf04(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2600.0;
			break;
		case 27:
			cf05(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2700.0;
			break;
		case 28:
			cf06(&x[i*nx],&f[i],nx,OShift,M,1,TestMode);
			f[i]+=2800.0;
			break;
		case 29:
			cf07(&x[i*nx],&f[i],nx,OShift,M,SS,1,TestMode);
			f[i]+=2900.0;
			break;
		case 30:
			cf08(&x[i*nx],&f[i],nx,OShift,M,SS,1,TestMode);
			f[i]+=3000.0;
			break;
		default:
...
void weierstrass_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Weierstrass's  */
{
	...
	for (i=0; i<nx; i++)
	{
		...
		for (j=0; j<=k_max; j++)
		{
			sum += pow(a,j)*cos(2.0*localPI*pow(b,j)*(z[i]+0.5));
			sum2 += pow(a,j)*cos(2.0*localPI*pow(b,j)*0.5);
		}
		f[0] += sum;
	}
	f[0] -= nx*sum2;
...
void griewank_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Griewank's  */
{
	...
	//MIT:
	if(nx<=100) sr_func (x, z, nx, Os, Mr, 600.0/100.0, s_flag, r_flag); /* shift and rotate */
	...
	for (i=0; i<nx; i++)
	{
	  ...
	  if(nx<=100){
		...
		//MIT (using untransformed coordinates -> min{f}=0+700 at x=0):
		s += x[i]*x[i];
...
void rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Rastrigin's  */
{
	...
	for (i=0; i<nx; i++)
	{
		f[0] += (z[i]*z[i] - 10.0*cos(2.0*localPI*z[i]) + 10.0);
	}
}
...
void cf01 (double *x, double *f, int nx, double *Os,double *Mr,int r_flag, /*MIT*/int TestMode) /* Composition Function 1 */
{
	int i,cf_num=5;
	double fit[5];
	...
	fit[i]=10000*fit[i]/1e+4;	if(TestMode==1) std::cout << "cf01 fit[" << i << "] = " << fit[i] << std::endl;
	i=1;
	...
	fit[i]=10000*fit[i]/1e+30;	if(TestMode==1) std::cout << "cf01 fit[" << i << "] = " << fit[i] << std::endl;
	i=3;
...
void cf_cal(double *x, double *f, int nx, double *Os,double * delta,double * bias,double * fit, int cf_num, /*MIT*/int TestMode)
{
	...
	{
		fit[i]+=bias[i];
		if(TestMode==1) std::cout << "cf_cal fit+bias[" << i << "]: " << fit[i] << std::endl;
		w[i]=0;
		for (j=0; j<nx; j++)
		{
